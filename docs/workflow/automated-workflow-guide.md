...
The dashboard is generated by `consolidated-report.js` which creates an HTML/CSS report from workflow data. When modifying the dashboard, follow these best practices:

### HTML Content in JavaScript

When embedding HTML in JavaScript, be aware of these patterns:
...
The workflow uses ECMAScript modules which have stricter parsing rules than CommonJS:
...
- If you encounter errors during dashboard generation, check for syntax issues in `scripts/workflow/consolidated-report.js`
...
1. Modify `workflow/consolidated-report.js`
...
13. **Dashboard Modifications**: When modifying the dashboard in consolidated-report.js, use string concatenation for complex HTML/SVG content to avoid ESM parsing issues 